...
| A6 | [ ] 界面缩放与字体在多 DPI 显示器上正常 | 100% / 150% / 200% 缩放下各看一遍；「界面缩放」0.8~1.6 各试一档 | ⚠ 真机未验证；已用 `QT_SCALE_FACTOR=1.5/2.0` 跑完整套用例（见 6.2 末尾），并据此修掉了"最小尺寸超过屏幕"这条真 bug |
...
### 6.2 主区改版相关（2026-09-20：对话上右栏 / 工具区进「控制台」/ 选中引用 / 接受并重跑）

| # | 条目 | 怎么测 | Linux 状态 |
| --- | --- | --- | --- |
| B1 | [ ] 底栏「控制台」按钮点开弹窗，5 个页签都在 | 点底栏左侧「控制台」：页签顺序应为 运行 / 历史运行 / 模板库 / 环境自检 / 设置，底部「关闭」可点 | ✅ 已验（几何 + 可见性用例） |
| B2 | [ ] 弹窗在小屏/高缩放下不出界 | 显示缩放设 150%，弹窗整块应在屏幕内、底部按钮可点（弹窗按屏幕可用区收敛） | ✅ 已用 `QT_SCALE_FACTOR=1.5/2.0` 模拟验（纯函数 + 真打开一次量尺寸） |
| B3 | [ ] 运行操作都在弹窗「运行」页 | 开始 / 取消 / 继续修复 / 改后重跑 / 打开运行目录 五个按钮都可见可点 | ✅ 已验 |
| B4 | [ ] 右列两个页签文字不被截断 | 「模型对话 / 校验与输出」在微软雅黑下应显示完整 | ⚠ 要你在 Windows 上看一眼（字体度量与本机不同） |
| B5 | [ ] 选中代码 → 右键「就选中的代码提问」 | 中栏选中几行 → 右键 → 第一项就是它；点后右列自动切到对话、输入框上方出现引用条 | ✅ 已验（含整窗链路用例） |
| B6 | [ ] 引用随消息一起发出、发完即撤 | 输入问题按 Ctrl+Enter：对话记录里能看到引用正文，引用条消失 | ✅ 已验 |
| B7 | [ ] 模型提出脚本 → 中栏红绿 diff + 三个按钮 | 让任一端改脚本：「接受」只放进中栏，「接受并重跑」接着走 shellcheck 与人工确认 | ✅ 已验（含"必须走 verify_edited"的用例） |
| B8 | [ ] 窄窗口下底部控件折行而不是重叠 | 窗口拖到最小宽度（或三栏分割条往右拖到底）：模型下拉不许盖住「可用模型」 | ✅ 已验（折行用例 + 960/720 宽下的几何审计） |
| B9 | [ ] 150% 缩放下左右两栏都不被挤扁 | 150% 缩放下看左栏文字是否出框、右栏对话是否被压 | ✅ 已用 `QT_SCALE_FACTOR=1.5` 跑完整套用例（见下方"高 DPI 形状怎么验"） |

> **高 DPI 形状怎么在没有 Windows 的机器上验**：`QT_SCALE_FACTOR=1.5`（或 `2`）跑同一套用例 ——
> 逻辑屏幕像素会按比例缩小（800x800 的测试屏在 150% 下只有 533x533），
> `window_minimum_for` / `console_dialog_size_for` 这类"按屏幕收敛"的规则就会走到小屏分支。
> 2026-09-20 就是这么做时发现了一条**真 bug**：最小尺寸 720x520 比 533x533 的屏幕还宽 ——
> 窗口管理器照给，布局只能违反最小尺寸（就是用户报过的"文字出框 / 面板重叠 / 某栏被压没"）。
> 命令：`QT_QPA_PLATFORM=offscreen QT_SCALE_FACTOR=1.5 .venv/bin/python -m pytest -o addopts="" -q`
> （按坐标取色的用例都走 `tests/conftest.py` 的 `Grab`，它按 devicePixelRatio 换算，
> 否则 150% 下会取到行号区/别的控件的颜色而假红。）
