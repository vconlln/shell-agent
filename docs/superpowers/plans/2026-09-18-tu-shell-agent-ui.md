...
- Windows 上的 exe 打包与双击启动；`taskkill /T /F` 的真实行为；中文/含空格路径；UTF-8 输出；opencode 原生 `serve`；权限 deny 实测（规格 §14 清单，逐条抄进了 `packaging/build.md`）。
- 单文件便携 exe：本计划只出 one-folder（命令写在 build.md，标为未实现/未验证）。
- 视觉打磨（主题、图标、字号）不在本计划范围；先把信息结构做对。

### 实现完成后复核出的缺口（不假装已覆盖）

下面几项是逐任务独立评审时对照规格复核出来的，**当前实现里没有**，写在这里是为了不让
"§12 → 任务 2/5/6/7/8/9"这种覆盖度声明把它们盖过去：

- **规格 §12 的"拖入方案"与"方案摘要"没有实现**。左栏只有"选择方案…"按钮 + 只读预览；
  全仓没有 `setAcceptDrops`/`dropEvent`。摘要（行数/字符数之类的概览）也没有。
- **大文件没有体积上限**：读方案（8 MB 实测阻塞界面约 2 秒）与执行输出（`output_view`
  无 `maximumBlockCount`，引擎侧也不截断）都是整份进内存 + 整份上屏。
  常规用法没问题，但"方案选错文件"或"脚本话很多"这两种误用会卡住界面。
- **多轮运行只保留最近一轮的上屏内容**（右栏 `render_*` 是覆盖语义），跨轮对比靠中栏
  的"对比上一轮"页签与历史回放；规格没有要求更多，但没有逐轮折叠的历史树。
- **`succeeded` 的语义仍只是"shellcheck 无阻断项 + 退出码 0"**（规格 §11）：界面把
  notes/assumptions 与脚本并排显示、并在执行摘要里写明这一点，但它不会、也无法自动判断
  "脚本是否真的实现了方案里的约束" —— 这一条永远需要人看那一屏。
