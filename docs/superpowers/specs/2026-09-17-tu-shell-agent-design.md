...
## §19 模型对话的会话持久化（2026-09-19 增补）

**起因**：用户问"模型对话能找到上次的会话吗，还是一个临时的"。查下来是后者 ——
对话会话 id 只存在 `RunController._session_id` 内存里，连对话记录都没落盘：
对话目录里只有 `.opencode/`（agent 文件）和 `server.log`，重启后新开一段、记录也清空。
（对比：**运行**那条路的 `sessionId` 早就写进了运行目录的 `meta.json`，所以「继续修复」
重启后还能接着跑。）

用户的要求不止"能恢复"，而是**按文件夹检索**："ai 在每个运行目录下会生成文件吗对吧，
让这个也可以检索文件夹的历史会话"。

### 落盘格式（`run_store/sessions.py`）

| 文件 | 内容 | 谁写 |
| --- | --- | --- |
| `<运行目录>/meta.json` | 除引擎写的字段外，追加 `sessionId` / `kind` / `model` | `write_session_meta()`（合并写，不覆盖 `config`/`detection`/`rounds`） |
| `<运行目录>/chat.jsonl` | 对话记录，一行一条 `{role, text, when}`，**追加写** | `append_chat()` |

选 JSONL 而不是 Markdown：正文里出现 `##`、代码围栏、多行文本都不会破坏解析；
坏行（写了一半的那条）跳过即可，读历史不该因为一行残缺就整段失败。
角色只有四种：`user` / `model` / `error` / `note`，与界面里的说话人一一对应。

### 目录检索

`scan_sessions(run_root)` 扫运行根目录下的每个子目录，读 `meta.json`：

- **只列有 `sessionId` 的**：没有 id 就无法让 opencode 接着那一段，列出来只会点了没反应；
- 缺 `meta.json`、坏 `meta.json`、不是目录 —— 一律跳过；
- 排序按**最近使用**（meta.json 与 chat.jsonl 的较新 mtime），不看目录名里的创建时间；
- `kind` 取 `meta.json` 的 `kind`（对话目录写 `"chat"`），没有就按"有没有轮次"推断。

### 界面

「模型对话」面板顶部新增一行：`会话 [下拉] [扫描历史会话] [新对话]`。

- 下拉项形如 `09-19 13:15 · 对话 · 4 条` / `09-19 12:12 · 运行 · 3 轮 · succeeded · deepseek/…`；
- 选中即切换：回填 `chat.jsonl` 里的记录，并把 serve 起在**那个目录**里
  （opencode 的会话按项目目录隔离，在别的目录起 serve 看不到那段会话 → `_ensure_serve_for`）；
- 提问时若当前没有会话，自动接上最近的一段**对话**；点过「新对话」则不再自动接
  （否则点了等于没点 —— 实现时踩到过，`_force_new_chat`）；
- 在「历史运行」里选中某次运行时，也接上那次的会话（`sessionId` 就在它的 `meta.json` 里），
  于是"看一眼历史"之后可以直接就着那一段问；
- 纯对话目录不再出现在「历史运行」列表里（没有轮次也没有脚本，列出来是一行空运行）。

### 已知边界

本次改动**之前**产生的对话目录里没有 `sessionId`，恢复不了（列不出来）。
opencode 自己的库里当然还有那些会话，但把"我们的目录"与"opencode 的会话"对上号缺少可靠依据，
所以不做猜测式补全。

### §19 补记：对话面板可以单独选模型

用户要求"这个模型对话，让我可以选择模型"。做法利用了 opencode 的一个事实：
**message 接口接受 `model: {providerID, modelID}`**（`/doc` 里的 schema 确认过），
所以换模型是按**每条消息**传的 —— 不必重启 serve，也不必改 agent 文件里的 `model:`。

- 「模型对话」面板顶部加了 `模型 [下拉] [可用模型]`：列表来自 `opencode models`
  （首次显示面板时才拉，子进程调用放线程里），可手输（列表取不到时也能用）；
- 优先级：**用户在下拉里的显式选择 > 设置里的 `opencode_model`**。
  （实现时踩到过：新会话把用户已选的模型覆盖成设置里的那个。）
