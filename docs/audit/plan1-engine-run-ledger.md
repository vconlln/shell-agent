...
## 追加（2026-09-18，Plan 2 期间发现的 Plan 1 缺陷）
**取消语义在引擎侧没有闭环**：`run_loop` 只在两处检查取消令牌（每轮开头、generate 抛异常后），
**执行前没有复查**。后果：用户在生成期间按取消、而 `generate` 正常返回时，引擎会一路走到执行，
脚本照样跑完并报 `succeeded` —— 在界面上表现为"我明明取消了，它还是跑了并说成功"。
适配器侧其实已读 cancel（`opencode_adapter/__init__.py` 三处），但 `OpencodePort` 是协议，
...
评审范围 `237d990..HEAD`（Plan 2 全部），5 个角度独立评审。修掉的见提交
`b879178`（Windows CRLF 阻断 / 跨运行假结论 / 关窗崩溃 / 续跑坏掉 / 回放缺证据），
这里记录**没有修**的部分，避免它们被下一次"全绿"盖过去：

1. **规格 §12 的"拖入方案"与"方案摘要"没有实现**（不是未验证，是不存在）——
...
5. **改后重跑与引擎共用 `attempts/<n>/`**：用户手改的脚本会覆盖引擎那一轮的
   证据（shellcheck.json/stdout.txt/execute.json），且 meta 里没有"这次是 verify"的标记 ——
   "谁写的这个脚本"在磁盘与界面上都无法追溯。属证据链问题，不是文案问题。
6. **`attempts/0/`**：start/探测/预检失败时写 `rounds: 0` 的产物，规格 §10 的布局只到 1..n。
7. **单文件便携 exe 未产出**（只出 one-folder），命令与"需要去掉 exclude_binaries"已写进 build.md。
