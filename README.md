...
两种"透明"效果都由**界面自绘底色**，窗口本身始终不透明：

- **半透明**：壁纸不模糊，只压一层淡色 —— 看起来像隔着玻璃看桌面；
- **亚克力模糊**：壁纸先做高斯模糊再压深色（强度可调）。

为什么不用系统能力：Windows 上 `WA_TranslucentBackground` 必须搭配无边框窗口才生效（我们保留
原生标题栏），而 Windows 11 的 DWM 亚克力要求窗口没有重定向位图（`WS_EX_NOREDIRECTIONBITMAP`），
Qt 的普通窗口有 —— 两条路在真机上都不成立（用户在 Windows 11 实测两种效果都没生效）。
自绘这条路不依赖合成器，**Windows 与 Linux 表现一致**。没有找到壁纸时退回纯色背景，并如实说明。
...
- 顶部是**会话**：下拉列出运行根目录下所有可恢复的会话（从每个目录的 `meta.json` +
  `chat.jsonl` 扫出来，按最近使用排序），可切换、可「扫描历史会话」、可「新对话」；
  重启后提问会**自动接上最近一段对话**，历史运行的会话也可以直接接续。
- 底部按钮行右侧是**模型**：本对话单独选模型（按条传给 opencode，切换不必重建会话），
  选择随会话落盘，下次恢复时带回。生成脚本用的模型在「设置 → 脚本生成」里单独设置。
...
- **必须显式指定模型（或在 opencode 里配好默认模型）**。opencode 未配置默认模型时会落到它自己的免费档（`opencode/*-free`），而免费档**只允许官方客户端调用** —— 经 `opencode serve` 的 HTTP API 调用会被拒绝，报 `OpenCode's free tier can only be used from within OpenCode`。这不是凭据问题：在「设置 → 脚本生成 → 模型」里点「检测可用模型」选一个已配置凭据的模型（例如 `deepseek/deepseek-v4-pro`）即可。失败时界面除了原始报错，还会附上这条处理建议。
- 免费额度下上游也可能拒绝"把工具全部 deny 的 agent"，表现为每轮生成都失败 —— 这是安全模型的代价，不会为了跑通去放宽权限。失败时界面会把落盘的错误证据（原始报错）摊在输出区，并标明它来自哪个文件。
